...
filepath = "dbscanresults_WALL.csv"
eps.steps <- seq(0.1, 0.8, 0.01)
...
write.table(x=exp.res[which(exp.res$F1 %in% f1.scores),],file="DBSCAN_F1_Scores_Wall.csv",
            sep=",")
...
plot <- ggplot(exp.res, aes(x=Recall, y=Precision)) +
            ...
            facet_wrap(~minPts)
ggsave(filename="dbscanPrecRecPlotWall.pdf", plot=plot, device="pdf",
       height = 15,
       width = 20,
       units = "cm")
