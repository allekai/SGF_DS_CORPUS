...
if [ ! -d "data/data_splits" ]; then
    mkdir data/data_splits
fi

for file in data/*_data.txt
do
    mkdir data/data_splits/"${file:5:3}"
    cat $file | head -1 >> heading.txt
    cp heading.txt data_splits/${file:5:3}/
    sed -i '1d' $file 
    ...
    paste -d "," *_out.txt >> ${file:5:3}_mcde.csv
    rm *_out.txt *.log

    mv Data* data_splits/${file:5:3}/
    for dat in ./data_splits/${file:5:3}/Data*
    do
        cat data_splits/${file:5:3}/heading.txt | cat - $dat > temp && mv temp $dat
    done
