...
do
    ...
    for dat in Data*
    do
        java -jar ~/Documents/GitHub/mcde/target/scala-2.11/MCDE-1.0.jar -t EstimateDependency -f $dat -a MWP -m 500 -d 0,1,2,3,4 | tail -4 | head -1 >> MWP_out.txt
        for measure in "${arr[@]}"
        do 
            java -jar ~/Documents/GitHub/mcde/target/scala-2.11/MCDE-1.0.jar -t EstimateDependency -f $dat -a $measure -d 0,1,2,3,4 | tail -4 | head -1 >> ${measure}_out.txt
        done
    done
