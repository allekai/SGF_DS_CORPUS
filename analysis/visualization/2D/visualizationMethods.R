...
getStream.5D <- function(dep = "Linear", fix = T) {
    conf.5D <- generate.stream.config(dim=10,
                                      mindim = 5,
                                      maxdim = 5,
                                      dependency = dep,
                                      nstep = 5)
    conf.5D$subspaceslist <- rep(list(list(1:5)),
                                      length(conf.5D$subspaceslist))
    if (fix) {
        conf.5D$marginslist <- as.list(c(0.9, 0.5, 0.1, 0.5, 0.9))
    } else {
        conf.5D$marginslist <- as.list(sample(c(0.1, 0.5, 0.9),
                                              length(conf.5D$marginslist),
                                              replace = TRUE))
    }

    stream.5D <- generate.dynamic.stream(n = 1000, method="Construction",
                                         stream.config=conf.5D) 
    stream.5D
}
